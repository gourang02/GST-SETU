...
export default function Sidebar({ current, onNavigate, isOpen, onClose }: Props) {
  ...
  return (
    <>
      ...
      className={`sidebar-aside ${isOpen ? 'mobile-open' : ''}`}
      >
        {/* Logo */}
        <div style={{ padding: '20px 18px 16px', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {onClose && (
            <button
              onClick={onClose}
              style={{ background: 'none', border: 'none', color: '#9BA69C', cursor: 'pointer', padding: 4 }}
              className="md:hidden"
            >
              ✕
            </button>
